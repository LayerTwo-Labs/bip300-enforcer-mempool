...
use crate::{
    cusf_enforcer::{self, CusfEnforcer, InitialBlockTemplate},
    mempool::MempoolSync,
};
...
#[educe(Debug(bound()))]
#[derive(Error)]
enum BuildBlockError<Enforcer>
where
    Enforcer: CusfEnforcer,
{
    #[error(transparent)]
    InitialBlockTemplate(Enforcer::InitialBlockTemplateError),
    #[error(transparent)]
    ...
    #[error(transparent)]
    SuffixTxs(Enforcer::SuffixTxsError),
}

// select block txs, and coinbase txouts if coinbasetxn is set
fn block_txs<const COINBASE_TXN: bool, Enforcer>(enforcer: &Enforcer, mempool: &crate::mempool::Mempool)
    -> Result<
            (<typewit::const_marker::Bool<COINBASE_TXN> as crate::cusf_enforcer::CoinbaseTxn>::CoinbaseTxouts,
             Vec<BlockTemplateTransaction>),
            BuildBlockError<Enforcer>
        >
    where Enforcer: CusfEnforcer,
    typewit::const_marker::Bool<COINBASE_TXN>: cusf_enforcer::CoinbaseTxn
     {
    let mut initial_block_template =
        InitialBlockTemplate::<COINBASE_TXN>::default();
    initial_block_template = enforcer
